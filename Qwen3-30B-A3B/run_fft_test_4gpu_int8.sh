...
# Configuration: AMXINT8 + 4x RTX 4090 GPU
...
#
# Usage:
#   bash run_fft_test_4gpu.sh [--skip-phase1] [--skip-phase4] [--only-phase4]
...
#   Phase 1  — Basic validation (3 steps): verify AMXINT8 FFT pipeline
#   Phase 4  — Stability + accurate TPS benchmark (default 50 steps)
#
# Key parameters (vs AMXBF16 run_fft_test.sh):
#   Backend     : AMXINT8 (INT8 quantized expert weights on CPU, NUMA2 layout)
#   GPUs        : 4x RTX 4090, fixed — no OOM fallback
#   INT8 path   : /mnt/data3/models/Qwen3-30B-A3B-AMXINT8-NUMA2
...
# BF16 HF weights: tokenizer, config, non-expert params
MODEL_PATH_BF16="/mnt/data3/models/Qwen3-30B-A3B"
# INT8 KTransformers quantized weights (AMXINT8, NUMA2 layout)
MODEL_PATH_INT8="/mnt/data3/models/Qwen3-30B-A3B-AMXINT8-NUMA2"

# 4-GPU AMXINT8 accelerate config (fixed, no fallback)
ACCEL_CONFIG="${CONFIGS_DIR}/accelerate_fft_amxint8_4gpu.yaml"
TRAIN_CONFIG_BASE="${CONFIGS_DIR}/train_fft_qwen3_30b.yaml"

NUM_GPUS=4
BACKEND_LABEL="AMX_INT8"
...
MONITOR_FIFO="/tmp/fft_monitor_events_4gpu.fifo"
MONITOR_PID=""
...
check_env() {
    ...
    if [[ ! -d "${MODEL_PATH_BF16}" ]]; then
        error "BF16 model path not found: ${MODEL_PATH_BF16}"
        exit 1
    fi
    ok "BF16 model (tokenizer/config): ${MODEL_PATH_BF16}"

    if [[ ! -d "${MODEL_PATH_INT8}" ]]; then
        error "INT8 weight path not found: ${MODEL_PATH_INT8}"
        exit 1
    fi
    ok "INT8 weight path (AMXINT8 NUMA2): ${MODEL_PATH_INT8}"
...
#
# AMXINT8 NUMA2 memory map:
#
#   GPU (non-expert, BF16, sharded across 4 GPUs via FSDP):
#     embed_tokens   : V × H
#     lm_head        : H × V  (not tied in Qwen3-30B-A3B)
#     attention × L  : q(H×nh×dh) + k(H×nkv×dh) + v(H×nkv×dh) + o((nh×dh)×H)
#     shared_expert × L : gate+up+down = 3 × H × I
#     router × L     : H × E
#     rms_norm × L   : tiny
#
#   CPU (expert weights, INT8, 1 byte/param, NUMA2 = 2 replicas):
#     128 experts × 3 projections × H × moe_I × 48 layers (INT8)
#     NUMA sharding: each NUMA node holds ½ output columns per projection
#     Scale factors (FP32): 1 scale per 32 params along quantization axis
#
#   CPU (training buffers, FP32):
#     Expert gradients   : same param count × 4 bytes
#     Adam 1st moment m  : same param count × 4 bytes
#     Adam 2nd moment v  : same param count × 4 bytes
# --------------------------------------------------------------------------- #
analyze_weight_memory() {
    phase_banner "Phase 0a: Weight Memory Breakdown (AMXINT8 NUMA2, 4-GPU FSDP)"

    "${PYTHON}" - "${MODEL_PATH_BF16}" "${MODEL_PATH_INT8}" "${NUM_GPUS}" <<'PYEOF'
import json, os, sys

bf16_path = sys.argv[1]
int8_path  = sys.argv[2]
num_gpus   = int(sys.argv[3])

cfg_path = os.path.join(bf16_path, "config.json")
if not os.path.exists(cfg_path):
    print(f"  [WARN] config.json not found at {bf16_path}")
    sys.exit(0)
...
p_sh_L    = 3 * H * I * L        # shared expert gate+up+down × 48 layers
p_router_L= H * E * L
p_norm_L  = 2 * H * L + 2 * H   # input + post_attn norm per layer, + 2 global

p_gpu_total = p_embed + p_lmhead + p_attn_L + p_sh_L + p_router_L + p_norm_L

print("=" * 64)
print(f"  Qwen3-30B-A3B  (qwen3_moe)")
...
# ---- Expert weights (INT8) ----
# gate+up+down each: H × moe_I per expert
# NUMA sharding: 2 nodes, each with ½ output cols → file size = moe_I × (H/2)
p_exp_int8 = E * 3 * H * moe_I * L      # total INT8 bytes
scale_bytes = p_exp_int8 / 32 * 4       # FP32 scales, 1 per 32 quant group

print("\n── CPU Expert Weights (AMXINT8, NUMA2 = 2 NUMA replicas) ────")
...
print(f"  All {L} layers: {p_exp_int8/1e9:.2f} GB INT8  (quant weights, 1B/param)")
print(f"  Scale factors (FP32, 1/32 density): {scale_bytes/1e9:.2f} GB")
print(f"  Total expert store:  {(p_exp_int8+scale_bytes)/1e9:.2f} GB")
print(f"  Per NUMA node (½ output cols): {(p_exp_int8+scale_bytes)/2/1e9:.2f} GB")

exp_down_numa = moe_I * (H // 2)
print(f"\n  [Sanity] down_proj per NUMA file = {moe_I}×{H//2} = {exp_down_numa} B", end="")
print(f"  {'✓ matches' if exp_down_numa == 786432 else '✗ mismatch'} (observed 786432)")

# ---- CPU training buffers (FP32) ----
grad_bytes  = p_exp_int8 * 4
adam_m      = grad_bytes
adam_v      = grad_bytes
cpu_total   = (p_exp_int8 + scale_bytes) + grad_bytes + adam_m + adam_v

print("\n── CPU Training Buffers (FFT backward, FP32) ────────────────")
print(f"  Expert FP32 gradients:    {grad_bytes/1e9:.1f} GB  ({p_exp_int8/1e6:.0f}M params × 4B)")
...
print(f"  {'─'*64}")
print(f"  {'Expert INT8 weights + scales':<38} {(p_exp_int8+scale_bytes)/1e9:>7.1f}G  CPU (both NUMA)")
...
analyze_dataset_truncation() {
    ...
    "${PYTHON}" - "${MODEL_PATH_BF16}" "${DATA_DIR}/fft_stress_100.json" "${CUTOFF_LEN}" <<'PYEOF'
import json, sys, os
...
# --------------------------------------------------------------------------- #
estimate_resources() {
    phase_banner "Resource Estimation (AMXINT8 NUMA2, 4x RTX 4090)"

    echo -e "${BOLD}== Model (Qwen3-30B-A3B) ==${NC}"
    ...
    echo "  Quantization : AMXINT8 (INT8 expert weights on CPU)"
    echo "  INT8 path    : ${MODEL_PATH_INT8}"
    echo ""
    echo -e "${BOLD}== FSDP (4x RTX 4090, 48 GB each) ==${NC}"
    echo "  Config: $(basename ${ACCEL_CONFIG})"
    echo "  FSDP-v2, reshard_after_forward=true"
    echo "  Expert weights remain on CPU; GPU holds non-expert params only"
    echo ""
...
# --------------------------------------------------------------------------- #
# Real-time system monitor (GPU VRAM + CPU RAM, 2 s interval)
# --------------------------------------------------------------------------- #
start_monitor() {
...
# LLaMA-Factory calls trainer.save_model() at end of run_sft() regardless of
# save_strategy. For Qwen3-30B-A3B the checkpoint is large; delete immediately.
# --------------------------------------------------------------------------- #
...
# Key overrides applied to every phase:
#   model_name_or_path → BF16 model (for tokenizer + config)
#   kt_weight_path     → AMXINT8 NUMA2 weights
...
make_phase_config() {
    ...
    # Always patch paths and cutoff for INT8 4-GPU run
    sed -i "s|output_dir: .*|output_dir: ${phase_dir}/model_output|g" "${cfg}"
    sed -i "s|model_name_or_path: .*|model_name_or_path: ${MODEL_PATH_BF16}|g" "${cfg}"
    sed -i "s|kt_weight_path: .*|kt_weight_path: ${MODEL_PATH_INT8}|g" "${cfg}"
...
# --------------------------------------------------------------------------- #
# Post-phase log analysis (loss, grad_norm, NaN, KT errors)
# --------------------------------------------------------------------------- #
analyze_log() {
    local phase_name="$1"
...

    {
        ...
        grep -qi "ddp_timeout\|timeout expired" "${log_file}" && echo "⚠ DDP timeout" || true
    } | tee "${result_file}"
}

# --------------------------------------------------------------------------- #
# Phase 1: Basic validation — 3 steps, no checkpoint, AMXINT8
# --------------------------------------------------------------------------- #
run_phase1() {
    phase_banner "Phase 1: Basic Validation (3 steps, AMXINT8 4-GPU)"
    ...
    log "Goal: verify AMXINT8 FFT pipeline init/forward/backward (3 steps, no crash)"
    log "Config: ${cfg}"

    local exit_code=0
    run_train "phase1" "3-step basic validation (AMXINT8 4GPU)" "${cfg}" || exit_code=$?
    ...
    if [[ "${exit_code}" -eq 0 ]]; then
        ...
        if [[ -f "${log_f}" ]]; then
            grep -qi "segmentation fault\|sigsegv" "${log_f}" && \
                error "  → SIGSEGV: C++ buffer OOB or index bug"
            grep -qi "nan\|inf" "${log_f}" && \
                error "  → NaN/Inf: INT8 dequant or gradient instability"
            grep -qi "out of memory" "${log_f}" && \
                error "  → CUDA OOM: reduce batch or check non-expert GPU allocation"
            grep -qi "kt_weight\|amxint8\|weight_path" "${log_f}" && \
                error "  → INT8 weight load error: check ${MODEL_PATH_INT8}"
        fi
    fi
    analyze_log "phase1"
...
# --------------------------------------------------------------------------- #
run_phase4() {
    phase_banner "Phase 4: Stability + TPS Benchmark (${PHASE4_STEPS} steps, AMXINT8 4-GPU)"
    ...
    log "Backend  : AMXINT8  |  INT8 weights: ${MODEL_PATH_INT8}"
    ...
    local exit_code=0
    run_train "phase4" "stability + TPS (${PHASE4_STEPS} steps, 4GPU AMXINT8)" "${cfg}" \
        || exit_code=$?
    ...
    {
        echo "=== Phase 4 Performance Analysis (AMXINT8 4-GPU) ==="
...
PYEOF
        ...
        echo ""
        echo "--- update_base_weights (INT8 re-quantize overhead per step) ---"
        local uc
...
generate_summary() {
    ...
    {
        echo "# Qwen3-30B-A3B FFT Test — AMXINT8 4-GPU"
        echo ""
        ...
        echo "**INT8 weights** : ${MODEL_PATH_INT8}"
        ...
        echo ""
        echo "## TPS (AMXINT8, 4 GPUs, cutoff_len=${CUTOFF_LEN})"
        echo ""
        echo "\`\`\`"
...
main() {
    echo ""
    echo -e "${BOLD}Qwen3-30B-A3B KTransformers FFT — AMXINT8 4-GPU${NC}"
    echo -e "Time      : $(run_time_display)"
    echo -e "Run label : ${RUN_LABEL}"
    echo -e "GPUs      : ${NUM_GPUS}  |  cutoff_len: ${CUTOFF_LEN}  |  warmup_skip: ${WARMUP_SKIP}"
    echo -e "Backend   : AMXINT8 NUMA2  |  ${MODEL_PATH_INT8}"
    echo ""

    check_env
    estimate_resources

    # Phase 0: pre-flight analysis — always runs, output to terminal and log files
    analyze_weight_memory
    analyze_dataset_truncation

    setup_run_dir

    # Save pre-flight analysis to log dir for future reference
